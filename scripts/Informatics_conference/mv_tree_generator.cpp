...
class TreeGenerator {
  ...
  public:
    ...
    std::unique_ptr<Node> generate_with_backbone() {
        ...
        auto root = std::make_unique<Node>(global_id++, 0, random_range(min_slots, max_slots));
        ...
        for (int d = 0; d < max_depth; ++d) {
            ...
            current->slots[pos] = std::make_unique<Node>(global_id++, d + 1, new_slots);
        ...
        for (int depth = 0; depth < max_depth; ++depth) {
            ...
            for (Node* node : frontier) {
                ...
                for (int i = 0; i < create_count; ++i) {
                    ...
                    node->slots[pos] = std::make_unique<Node>(global_id++, depth + 1, new_slots);
    ...
    static void preorder(const Node* node) {
        ...
        std::cout << "Node " << node->id << " | depth: " << node->depth << " | children: ";
    ...
    export_level_structure_as_config(const Node* root, const std::string& pla_dir,
        ...
        while (! q.empty()) {
            ...
            std::string pla_path = pla_dir + "/M" + std::to_string(current->id) + ".pla";
...
int main(int argc, char* argv[]) {
    ...
    if (min_sons < 0 || max_sons < 0 || min_slots <= 0 || max_slots <= 0 || min_sons > max_sons ||
        min_slots > max_slots || max_depth <= 0 || seed < 0 || pla_dir.empty() ||
        num_states <= 0) { // <-- Add validation
        std::cout << "Invalid parameters.\n";
        return 1;
    }

    TreeGenerator generator(max_depth, min_sons, max_sons, min_slots, max_slots, seed);
    auto root = generator.generate_all_random();

    // Pass num_states into the export function
    std::string config_output =
        TreeGenerator::export_level_structure_as_config(root.get(), pla_dir, num_states);
    ...
    std::function<void(const Node*)> count_nodes_and_slots = [&](const Node* node) {
        if (! node)
            return;
        node_count++; // Count the current node
        variable_count += node->slot_count; // Add the number of slots (variables)
        for (const auto& child : node->slots) {
